...
site <- "https://content.cruk.cam.ac.uk/fmlab/sivakumar2016/KRAS-MasterRegulators-S1/"
fnames <- c("dependencies/hgu133plus2hsentrezg.db_19.0.0.tar.gz",
            ...
            "data/KRAS-master-regulators-S1_data.RData",
...
load("data/KRAS-master-regulators-S1_data.RData")
@
...
Packages <- c("affy","affyPLM","knitr","sva","made4","devtools","xtable","Rsubread","igraph","reshape2","limma","biomaRt","corpcor", "fdrtool","viper","RedeR","ggplot2","gridExtra","pheatmap","HTSanalyzeR2","org.Mm.eg.db","org.Hs.eg.db","GO.db","KEGGREST","snow","cluster","survival","rms", "sigPathway","RColorBrewer","clusterProfiler","ReactomePA")
...
Raw gene expression data files were obtained from Gene Expression Omnibus (GEO) (Edgar et al., 2002) for a total of five independent pancreatic-related studies. All five downloaded data sets were generated on Affymetrix GeneChip® Human Genome U133 Plus 2.0 array. 
...
To identify enriched pathways and gene ontologies in the KRAS signature we performed a gene set enrichment analysis using the R package \Biocpkg{HTSanalyzeR2}. For the pathway analysis we used Kyoto Encyclopedia of Genes and Genomes (KEGG) and two GO
...
\caption{Pathway analysis of KRAS differentially expressed genes. The plot displays the enrichment map obtained by HTSanalyzeR2, for gene set collections obtained from Biological Processes (BP) and Molecular Function (MF) Gene Ontologies.}
\label{enrichmap1}
\end{center}
\end{figure}
...
\caption{Pathway analysis of KRAS differentially expressed genes. The plot displays the enrichment map obtained by using HTSanalyzeR2, for gene set collections obtained from KEGG GENES database.}
\label{enrichmap2}
\end{figure}
\end{center}
...
print(xtab, floating=FALSE)
@
\caption{Table 4: Pathway GSEA analysis of KRAS differentially expressed genes. Obtained by with HTSanalyzeR2 for gene set collections obtained from KEGG GENES database. The table shows significantly enriched KEGG pathways (adjusted p-value \textless 0.05 ) according to both, the GSEA and Hypergeometric test.}
\vspace{2 mm}
...
\item{Group2 is enriched in ECM−receptor interaction, Focal adhesion and "Hedgehog/Wnt" signaling genes. } 
...
\caption{Heatmap showing the similarity between the samples in the ICGC cohort as measured by 'signature distance' distance between the MR’s activity profiles}
\end{center}
\end{figure}
...
\caption{Kaplan–Meier survival curves of the different tumor subgroups using the ICGC cohort. Numbers of subjects at risk at the start of each time interval are shown above the x-axis.}
\end{center}
\end{figure}
...
\caption{Kaplan–Meier survival curves of the different tumor subgroups using the TCGA cohort for subsets of individual that did or did not receive adjuvant targeted therapy treatment. Numbers of subjects at risk at the start of each time interval are shown above the x-axis.}
\end{center}
\end{figure}
...
Johnson WE, Li C, Rabinovic A. Adjusting batch effects in microarray data using empirical bayes methods. Biostatistics, 8(1):118–127, 2007
